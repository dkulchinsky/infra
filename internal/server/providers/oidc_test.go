...
func TestValidate(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "invalid URL",
			provider: NewOIDCClient(models.Provider{Kind: models.ProviderKindOIDC, URL: "example.com"}, "some_client_secret", "not-used", "http://localhost:8301"),
		...
		},
		{
			...
			provider: NewOIDCClient(models.Provider{Kind: models.ProviderKindOIDC, URL: serverURL, ClientID: "invalid-client-id"}, "some_client_secret", "not-used", "http://localhost:8301"),
			tokenResponse: tokenResponse{
				code: 500,
		...
		},
		{
			...
			provider: NewOIDCClient(models.Provider{Kind: models.ProviderKindOIDC, URL: serverURL, ClientID: "client-id"}, "some_client_secret", "not-used", "http://localhost:8301"),
			tokenResponse: tokenResponse{
				code: 500,
...
func TestExchangeAuthCodeForProviderToken(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			provider: NewOIDCClient(models.Provider{Kind: models.ProviderKindOIDC, URL: serverURL, ClientID: "invalid"}, "invalid", "not-used", "http://localhost:8301"),
			tokenResponse: func(t *testing.T) tokenResponse {
...
func TestRefreshAccessToken(t *testing.T) {
	server, ctx := setupOIDCTest(t, "")
	serverURL := server.run(t, nil)
	provider := NewOIDCClient(models.Provider{Kind: models.ProviderKindOIDC, URL: serverURL, ClientID: "whatever"}, "secret", "not-used", "http://localhost:8301")
